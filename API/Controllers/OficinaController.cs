...
public class OficinaController : BaseApiController
{
    ...
    [HttpGet("Consulta26Left")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<IEnumerable<object>>> Get26Left()
    {
        var data = await unitOfWork.Oficinas.Consulta26Left();
        return mapper.Map<List<object>>(data);
    }
    [HttpGet("Consulta26NaturalLeft")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<IEnumerable<object>>> Get26NaturalLeft()
    {
        var data = await unitOfWork.Oficinas.Consulta26NaturalLeft();
        return mapper.Map<List<object>>(data);
    }
    [HttpGet("Pagination")]
    [MapToApiVersion("1.1")]
